# From: https://github.com/fengwangPhysics/matplotlib-chord-diagram
...
def chordDiagram(X, ax, colors=None, width=0.1, pad=2, chordwidth=0.7):
    ...
    # use d3.js category10 https://github.com/d3/d3-3.x-api-reference/blob/master/Ordinal-Scales.md#category10
        if len(x) <= 10:
            ...
            colors = ['#c71585','#7fffd4','#da70d6','#800080','#dda0dd','#00ffff','#ffa07a','#ff69b4','#bdb76b','#228b22','#4b0082','#afeeee','#ffdab9','#ffd700','#cd5c5c','#eee8aa','#f0e68c','#7b68ee','#556b2f','#4169e1','#66cdaa','#9932cc''#e0ffff','#7cfc00','#d8bfd8','#32cd32','#ee82ee','#ffe4b5','#00008b','#ff8c00','#8b0000','#008080','#e6e6fa','#808000','#00ff7f','#dc143c','#9400d3','#ffefd5','#ff00ff',]
        else:
            print("x is too large! Use x smaller than 40. You have " + str(len(x)))
...
def g(x):
    return np.floor(np.multiply(np.power(x,4),100))
#    return np.power(x,4)
...
def array_for_heat(x):
    return np.array([g(xi) for xi in x])

def draw(plot_name, similarity, names):
    fig = plt.figure(figsize=(6,6))
    similarity[similarity >= 0.999] = 0
    flux = array_for(similarity)
    ax = plt.axes([0,0,1,1])
	#nodePos = chordDiagram(flux, ax, colors=[hex2rgb(x) for x in ['#666666', '#66ff66', '#ff6666', '#6666ff']])
    nodePos = chordDiagram(flux, ax)
    ax.axis('off')
    prop = dict(fontsize=20*0.8/np.sqrt(len(names)), ha='center', va='center')
    nodes = names
    for i in range(len(names)):
	    ax.text(nodePos[i][0], nodePos[i][1], nodes[i], rotation=nodePos[i][2], **prop)
    plt.savefig("/Users/Ian/IMSA/Core/DigitalLiteraryStudies/Song Lyrics/QuarticAnalysis2/" + plot_name + ".png", dpi=600,
                 transparent=True,
                 bbox_inches='tight', pad_inches=0.02)


def heat(plot_name, similarity, names):
    plt.clf()
    similarity[similarity >= 0.999] = 0
    flux = array_for_heat(similarity)
    df = pd.DataFrame(similarity, index=names, columns=names)
    plt.figure(figsize = (10,10))
    ax = sns.heatmap(df, cmap="YlGnBu", annot=False, linewidths=.5)
    plt.savefig("/Users/Ian/IMSA/Core/DigitalLiteraryStudies/Song Lyrics/HeatMaps2/" + plot_name + ".png", dpi=600,
			transparent=True,
			bbox_inches='tight', pad_inches=0.02)
